...
/* * 逻辑宽度固定 390。 * 逻辑高度跟随手机真实宽高比， * 不再使用固定 390×844 居中缩放。 * * 这样不同长宽比手机都会铺满整个游戏区域， * 不会再出现左右留白。 */
...
/* * 不再画任何分区多边形。 * * 地点只使用地图上的轻量锚点。 * 坐标使用 0~1 的相对位置， * 因此手机屏幕高度变化以后， * 地点仍能跟随地图自适应。 * * 位置重新按地图视觉逻辑校正： * 大学城：左上体育场/校园建筑群 * 商业中心：中央高楼群 * 高新区：右上现代新区 * 老城区：左中旧居民区 * 城中村：中心外围低层密集住宅区 * 东门市场：左下生活区/街市附近 * 工业园：右中厂房、烟囱区域 */
...
function resizeCanvas() {
  ...
  screenWidth = Math.max(1, Number(info.windowWidth) || VIEW_W);

  screenHeight = Math.max(1, Number(info.windowHeight) || 780);

  /* * 高分辨率安卓机如果直接使用 3× / 4× DPR， * Canvas 面积会非常大。 * * 这里限制到 2×， * 对小游戏已经足够清晰， * 同时明显减轻动画重绘压力。 */
  pixelRatio = Math.min(2, Math.max(1, Number(info.pixelRatio) || 1));

  /* * 逻辑宽度固定， * 高度按照手机真实宽高比计算。 */
  ...
  const targetW = Math.max(1, Math.floor(screenWidth * pixelRatio));

  const targetH = Math.max(1, Math.floor(screenHeight * pixelRatio));
  ...
  /* * 地图高度变化以后， * 旧缓存失效。 */
  if (Math.abs(mapCacheHeight - MAP_H) > 1) {
    mapCache = null;
...
function drawImageFocus(target, image, dx, dy, dw, dh, zoom, focusX, focusY) {
  ...
  const fx = Math.max(0, Math.min(1, focusX == null ? 0.5 : focusX));

  const fy = Math.max(0, Math.min(1, focusY == null ? 0.5 : focusY));
...
function createOffscreenCanvas(width, height) {
  ...
  if (
    ...
    api &&
    typeof api.createCanvas === "function"
  ) {
    try {
...
function buildMapCache() {
  ...
  const c = createOffscreenCanvas(
    Math.max(1, Math.floor(MAP_W * cacheScale)),

    Math.max(1, Math.floor(MAP_H * cacheScale))
  );
  ...
  /* * V3 地图稍微减少放大， * 让城市道路关系更自然。 */
  drawImageFocus(cctx, image, 0, 0, MAP_W, MAP_H, 1.06, 0.52, 0.13);

  const gradient = cctx.createLinearGradient(0, 0, 0, MAP_H);

  gradient.addColorStop(0, "rgba(5,25,38,0.025)");

  gradient.addColorStop(0.68, "rgba(5,25,38,0.01)");

  gradient.addColorStop(1, "rgba(5,25,38,0.10)");
...
function drawTopHud() {
  if (!drawAtlas("hud", 0, 0, VIEW_W, TOP_H)) {
  ...
  drawText(cityName, 14, 18, 16, COLORS.white, "700");

  drawText("一座有味道的城市", 14, 39, 8, "rgba(255,255,255,0.70)", "500");

  drawText(display.date, 164, 13, 8, "#E9F0F4", "600");

  drawText(display.time, 164, 33, 18, COLORS.white, "700");

  const weather = WEATHER_NAMES[world.weather] || world.weather;

  drawText(
    weather + " " + world.temperature + "℃",
    164,
    53,
    9,
    "#E9F0F4",
  ...
  drawText("品牌 Lv.1", 376, 43, 9, "#F4F1E8", "600", "right");

  roundedRect(306, 55, 68, 7, 4, "rgba(255,255,255,0.16)");

  roundedRect(
    306,
    55,
    Math.max(7, Math.min(68, (player.reputation / 100) * 68)),
    7,
    4,
    COLORS.gold
  );
  ...
  for (let i = 0; i < speedItems.length; i++) {
    ...
    drawText(label, x + 14.5, buttonY + 9.5, 8, COLORS.white, "700", "center");

    addButton(id, x - 1, buttonY - 2, 31, 23);
  }
...
function drawMapBase() {
  ...
  if (image) {
    drawImageFocus(ctx, image, MAP_X, MAP_Y, MAP_W, MAP_H, 1.06, 0.52, 0.13);
...
function getDistrictColor(district) {
  if (district.saturation >= 90) {
    return COLORS.danger;
  }

  if (district.saturation >= 80) {
    return COLORS.orange;
  }

  if (district.saturation >= 65) {
    return COLORS.gold;
  }
...
/* * V3 地点标识： * * 不再使用地图定位针。 * 改成小圆形锚点， * 更像经营游戏中的可点击 POI。 */
function drawDistrictMarker(district) {
  ...
  if (selected) {
    ...
    ctx.arc(0, 0, 14 + districtFx.flash * 4, 0, Math.PI * 2);

    ctx.fillStyle = hexToRgba(color, 0.16 + districtFx.flash * 0.12);
  ...
  ctx.arc(0, 0, selected ? 8 : 7, 0, Math.PI * 2);

  ctx.fillStyle = "rgba(10,38,55,0.90)";
  ...
  ctx.arc(0, 0, selected ? 3.8 : 3.2, 0, Math.PI * 2);
  ...
  /* * 标签也缩小， * 不再使用大白色胶囊。 */
  const labelW = Math.max(38, district.name.length * 10 + 10);
  ...
  drawText(district.name, x, labelY + 9, 8, COLORS.white, "700", "center");

  /* * 视觉很小， * 点击区域仍然保持足够大， * 手机操作不会难点。 */
  addButton(
...
function drawGoalCard() {
  const compact = VIEW_H < 740;

  const x = 10;

  const y = MAP_Y + 10;

  const w = compact ? 166 : 176;

  const h = compact ? 44 : 48;

  if (!drawAtlas("goal", x, y, w, h)) {
    roundedRect(x, y, w, h, 12, "rgba(9,37,54,0.91)", "rgba(255,255,255,0.18)");
  }

  roundedRect(x + 9, y + 9, 23, h - 18, 7, "rgba(247,238,216,0.95)");

  drawText("✓", x + 20.5, y + h / 2, 14, "#D67E2B", "700", "center");

  drawText("经营目标", x + 40, y + 15, 10, "#FFE3A3", "700");

  drawText("开设第一家餐厅", x + 40, y + 32, 8, COLORS.white, "500");

  drawText("›", x + w - 12, y + h / 2, 19, "#FFE3A3", "700", "center");

  addButton("tool:goal", x, y, w, h);
}
...
function drawToolButton(x, y, item) {
  ...
  if (!drawAtlas("side", x, y, size, size)) {
    roundedRect(
      x,
      y,
      size,
  ...
  drawText(item.label, x + size / 2, y + 29, 7, COLORS.white, "600", "center");

  addButton("tool:" + item.id, x, y, size, size);
...
function drawSideTools() {
  ...
  for (let i = 0; i < LEFT_TOOLS.length; i++) {
    drawToolButton(7, top + i * gap, LEFT_TOOLS[i]);
  }

  for (let i = 0; i < RIGHT_TOOLS.length; i++) {
    const size = VIEW_H < 740 ? 39 : 42;

    drawToolButton(VIEW_W - size - 7, top + i * gap, RIGHT_TOOLS[i]);
  }
}
...
function drawMetricChip(x, y, w, icon, label, value, color) {
  ...
  roundedRect(x, y, w, h, 9, "rgba(255,255,255,0.48)");

  drawText(icon, x + 14, y + 14, 12, color, "700", "center");

  drawText(label, x + 27, y + 10, 7, COLORS.muted, "600");

  drawText(value, x + 27, y + 25, 9, COLORS.text, "700");
}
...
function drawDistrictCard() {
  ...
  if (!selectedDistrictId) {
    drawText("●", 29, y + 24, 15, COLORS.navy, "700", "center");

    drawText("请选择一个区域", 47, y + 22, 14, COLORS.text, "700");

    drawText("点击地图地点查看经营数据", 47, y + 41, 8, COLORS.muted, "500");

    const chipY = y + 58;

    drawMetricChip(16, chipY, 110, "人", "人口", "--", COLORS.blue);

    drawMetricChip(140, chipY, 110, "餐", "需求", "--", COLORS.danger);

    drawMetricChip(264, chipY, 110, "¥", "客单", "--", COLORS.green);
  ...
  drawText(district.name, 17, y + 21, 15, COLORS.text, "700");
  ...
  drawText(
    ...
    district.saturation >= 85 ? COLORS.danger : COLORS.muted,

    "600"
  );

  roundedRect(295, y + 10, 74, 29, 9, COLORS.gold);

  drawText("进入商圈 ›", 332, y + 24.5, 8, "#26343B", "700", "center");

  addButton("district:enter", 290, y + 6, 84, 36);
...
function drawBottomNav() {
  if (!drawAtlas("nav", 0, NAV_Y, VIEW_W, NAV_H)) {
  ...
  for (let i = 0; i < NAV_ITEMS.length; i++) {
    ...
    if (active) {
      if (
        !drawAtlas("navActive", i * cellW + 4, NAV_Y + 4, cellW - 8, NAV_H - 8)
      ) {
        roundedRect(
          i * cellW + 4,
          NAV_Y + 4,
...
const cityScene = {
  ...
  render() {
    ...
    for (let i = 0; i < districts.length; i++) {
      drawDistrictMarker(districts[i]);
    }
  ...
  handleTap(x, y, target) {
    ...
    if (
      target.id.indexOf("district:") === 0 &&
      target.id !== "district:enter"
    ) {
      const districtId = target.id.split(":")[1];

      selectedDistrictId = districtId;

      citySystem.setCurrentDistrict(districtId);

      startDistrictFx(districtId);
...
function handleTap(screenX, screenY) {
  const point = screenToDesign(screenX, screenY);
  ...
  if (!target) {
    ...
    if (scene && typeof scene.handleTap === "function") {
      if (scene.handleTap(point.x, point.y, null)) {
        render();
      }
    }
  ...
  if (target.id === "district:enter") {
    const district = selectedDistrictId
      ? citySystem.getDistrict(selectedDistrictId)
      : null;

    if (district) {
      selectedDistrictId = district.id;

      citySystem.setCurrentDistrict(district.id);
  ...
  if (target.id.indexOf("nav:") === 0) {
    ...
    if (sceneManager.switchTo(sceneId)) {
      render();
    }
  ...
  if (scene && typeof scene.handleTap === "function") {
    if (scene.handleTap(point.x, point.y, target)) {
      render();
    }
  }
}
...
function gameLoop(timestamp) {
  ...
  if (advancedMinutes > 0 || animationChanged || needsResize) {
    render();
  }
...
console.log("城市餐饮经营小游戏 V4 动态商圈找铺版启动成功");
